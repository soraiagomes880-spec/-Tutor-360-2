        ...
        <div className="glass-panel p-10 rounded-[2.5rem] border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 text-sm">
              {isForgotPassword ? 'Recuperar senha' : isSignUp ? 'Crie sua conta de aluno' : 'Acesse seu painel de estudos'}
            </p>
          </div>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            >
              {loading ? <i className="fas fa-spinner fa-spin"></i> : <i className={`fas ${isForgotPassword ? 'fa-paper-plane' : isSignUp ? 'fa-user-plus' : 'fa-right-to-bracket'}`}></i>}
              {isForgotPassword ? 'Enviar E-mail de Recuperação' : isSignUp ? 'Criar Conta Grátis' : 'Entrar na Plataforma'}
            </button>
          </form>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            {isForgotPassword ? (
              <button
                onClick={() => { setIsForgotPassword(false); setMessage(null); setError(null); }}
