...
export const Auth: React.FC = () => {
    ...
    if (!supabase) {
        setError('Cuidado! O Supabase não foi configurado. Clique 5 vezes no ícone de chapéu no topo da tela (ou no fundo) para configurar sua URL e Chave.');
        setLoading(false);
        return;
    }
        ...
        <div className="max-w-md w-full animate-in fade-in zoom-in-95 duration-500">
            <div className="glass-panel p-10 rounded-[2.5rem] border-white/10 shadow-2xl relative overflow-hidden">
                ...
                <div className="mt-8 flex flex-col gap-4 text-center">
                    {mode === 'login' ? (
                        <>
                            <button onClick={() => setMode('signup')} className="text-[11px] font-bold text-slate-500 hover:text-indigo-400 uppercase tracking-widest transition-colors">Ainda não tem acesso? Cadastre-se</button>
                            <button onClick={() => setMode('reset')} className="text-[11px] font-bold text-indigo-400/60 hover:text-indigo-400 uppercase tracking-widest transition-colors">Esqueceu a senha?</button>
                        </>
                    ) : (
                        <button onClick={() => setMode('login')} className="text-[11px] font-bold text-slate-500 hover:text-indigo-400 uppercase tracking-widest transition-colors">Voltar para o Login</button>
                    )}
                </div>
            </div>
        </div>
